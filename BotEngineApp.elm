...
import EveOnline.AppFramework
    exposing
        ...
        , useRandomMenuEntry
        , useRandomMenuEntryWithTextContaining
...
defaultBotSettings =
    ...
    , ratsTargetingRange = 40000
    , botStepDelayMilliseconds = 2000
    , oreHoldMaxPercent = 87
    , selectInstancePilotName = Nothing
    , returnToUnloadRemainingSessionLengthThresholdSeconds = 30
    }
...
parseBotSettings =
    AppSettings.parseSimpleListOfAssignments { assignmentsSeparators = [ ",", "\n" ] }
        ([ ( "run-away-shield-hitpoints-threshold-percent"
         ...
         , ( "rats-targeting-range"
           , AppSettings.valueTypeInteger (\range settings -> { settings | ratsTargetingRange = range })
           )
...
travelToMiningSiteAndTargetAsteroid : BotDecisionContext -> SeeUndockingComplete -> DecisionPathNode
travelToMiningSiteAndTargetAsteroid context seeUndockingComplete =
    {- if seeUndockingComplete.shipUI.offensiveBuffButtonNames |> List.member "electronic" then
           describeBranch "I see we are jammed. Warp to another site."
               (returnDronesToBay context.readingFromGameClient
                   |> Maybe.withDefault
    ...
    case context.readingFromGameClient |> topmostAsteroidFromOverviewWindow of
        ...
        Just asteroidInOverview ->
            describeBranch ("Choosing asteroid '" ++ (asteroidInOverview.objectName |> Maybe.withDefault "Nothing") ++ "'")
                (warpToOverviewEntryIfFarEnough context asteroidInOverview
                    |> Maybe.withDefault
                        (monitorForRatsInBelt context seeUndockingComplete)
                )


monitorForRatsInBelt : BotDecisionContext -> SeeUndockingComplete -> DecisionPathNode
monitorForRatsInBelt context seeUndockingComplete =
    case context.readingFromGameClient |> topmostNotAsteroidFromOverviewWindow of
        Nothing ->
            describeBranch "I see no rats in the overview."
                (returnDronesToBay context.readingFromGameClient
                    |> Maybe.withDefault
                        (warpToMiningSite context.readingFromGameClient)
                )
        ...
        Just notasteroidInOverview ->
            describeBranch ("Detected rat named '" ++ (notasteroidInOverview.objectName |> Maybe.withDefault "Nothing") ++ "'. Deploying combat measures.")
                (returnDronesToBay context.readingFromGameClient
                    |> Maybe.withDefault
                        (lockRatFromOverviewEntryAndEnsureIsInRange
                            context.readingFromGameClient
                            ...
                            |> Maybe.withDefault
                                (launchAndEngageCombatDrones context.readingFromGameClient
                                    |> Maybe.withDefault
                                        (describeBranch "Locked rat and launched drones." waitForProgressInGame)
                                )
                        )
                )
                    ...
                    (if shipManeuverIsApproaching readingFromGameClient then
                        ...
                        useContextMenuCascadeOnOverviewEntry
                            (useMenuEntryWithTextContaining "Orbit"
                                (useMenuEntryWithTextContaining "2500 m" menuCascadeCompleted)
                            )
                            overviewEntry
                            readingFromGameClient
                    )
...
lockRatFromOverviewEntryAndEnsureIsInRange : ReadingFromGameClient -> Int -> OverviewWindowEntry -> Maybe DecisionPathNode
lockRatFromOverviewEntryAndEnsureIsInRange readingFromGameClient rangeInMeters overviewEntry =
    case overviewEntry.objectDistanceInMeters of
        Ok distanceInMeters ->
            if distanceInMeters <= rangeInMeters then
                Just
    ...
    readingFromGameClient
        |> useContextMenuCascadeOnListSurroundingsButton
            (useMenuEntryWithTextContaining "locations"
                (useRandomMenuEntryWithTextContaining "-belt"
                    (useMenuEntryWithTextContaining "within 0 m" menuCascadeCompleted)
                )
            )
...
launchAndEngageMiningDrones : ReadingFromGameClient -> Maybe DecisionPathNode
launchAndEngageMiningDrones readingFromGameClient =
...
processEveOnlineBotEvent =
    EveOnline.AppFramework.processEveOnlineAppEventWithMemoryAndDecisionTree
        ...
        , millisecondsToNextReadingFromGame = .eventContext >> .appSettings >> .botStepDelayMilliseconds --+ ((getEntropyIntFromReadingFromGameClient readingFromGameClient) |> modBy 1500)
        }
...
topmostNotAsteroidFromOverviewWindow : ReadingFromGameClient -> Maybe OverviewWindowEntry
topmostNotAsteroidFromOverviewWindow =
    overviewWindowEntriesRepresentingNotAsteroids
        >> List.sortBy (.uiNode >> .totalDisplayRegion >> .y)
        >> List.head


overviewWindowEntriesRepresentingNotAsteroids : ReadingFromGameClient -> List OverviewWindowEntry
overviewWindowEntriesRepresentingNotAsteroids =
    .overviewWindow
        >> Maybe.map (.entries >> List.filter overviewWindowEntryRepresentsNotAnAsteroid)
        >> Maybe.withDefault []


overviewWindowEntryRepresentsNotAnAsteroid : OverviewWindowEntry -> Bool
overviewWindowEntryRepresentsNotAnAsteroid entry =
    (entry.textsLeftToRight |> List.any (String.toLower >> String.contains "asteroid") |> not)
        && (entry.textsLeftToRight |> List.any (String.toLower >> String.contains "belt") |> not)
